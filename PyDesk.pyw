...
def install_mod():
    ...
    def install_it():
        cmd = subprocess.run(f'pip install {E1.get()}', stdout=PIPE, stderr=PIPE)
        output = str(cmd.stdout)
        print(output)
        input()
        if 'EnvironmentError' in output:
            cmd = subprocess.run(f'pip install --user {E1.get()}')
            output = str(cmd.stdout)
